...
testCelltype_paired <- function(Mat, celltype, pairsOfTimes, 
                               size = 5, 
                               use_duplicates = FALSE,
                               cell_col = "population",
                               visit_col = "visit", 
                               subject_col = "subject",
                               value_col = "cell_type_frac_total") {
  if (!is.data.table(Mat)) {
    Mat <- as.data.table(Mat)
  }
  
  cMat = Mat[get(cell_col) == celltype & get(visit_col) %in% unique(unlist(pairsOfTimes))]
  
  if (use_duplicates) {
    # Dynamic aggregation using column names
    cMat <- cMat[, setNames(list(mean(get(value_col))), value_col), 
                 by = c(subject_col, visit_col, cell_col)]
  } else {
    duplicate_counts <- cMat[, .N, by = c(subject_col, visit_col, cell_col)]
    duplicate_combos <- unique(duplicate_counts[N > 1, c(subject_col, visit_col), with = FALSE])
    if (nrow(duplicate_combos) > 0) {
      cMat <- cMat[!paste(get(subject_col), get(visit_col)) %in% 
                   paste(duplicate_combos[[subject_col]], duplicate_combos[[visit_col]])]
    }
  }
  
  cMat$Value = cMat[[value_col]]
  
  time_points <- levels(cMat[[visit_col]])
  ...
  for (i in 1:length(pairsOfTimes)) {
    pairwise_data <- cMat[get(visit_col) %in% pairsOfTimes[[i]]]
    
    # Create formula for dcast using column names
    formula_str <- paste(subject_col, "~", visit_col)
    pairwise_data = dcast(pairwise_data, as.formula(formula_str), value.var = 'Value')
    
    # More robust column access - get the actual visit column names
    visit_cols <- names(pairwise_data)[names(pairwise_data) != subject_col]
    
    # Check we have exactly 2 visit columns as expected
    if (length(visit_cols) != 2) {
      warning(paste("Expected 2 visit columns, got", length(visit_cols), "for celltype", celltype))
      next
    }
    
    # Use the actual column names instead of numeric indices
    col1 <- visit_cols[1]
    col2 <- visit_cols[2]
    
    # Filter out rows with missing data
    pairwise_data = pairwise_data[!is.na(pairwise_data[[col1]]) & !is.na(pairwise_data[[col2]])]   
    
    subjects_with_pairs <- c(subjects_with_pairs, pairwise_data[[subject_col]])
    ...
    medDiff = median(pairwise_data[[col1]] - pairwise_data[[col2]])
    ...
    pairwise_results <- rbind(pairwise_results, data.frame(
      ...
      Time1Val = median(pairwise_data[[col1]]),
      Time2Val = median(pairwise_data[[col2]]),
      Log2FC = log2(median(pairwise_data[[col2]]) / median(pairwise_data[[col1]]))
    ))
  }
  
  cMat_plot <- cMat[get(subject_col) %in% unique(subjects_with_pairs)]
  
  p <- ggplot(cMat_plot, aes(x = .data[[visit_col]], y = .data[["Value"]], fill = .data[[visit_col]])) +
    geom_boxplot(color = "black") +
  geom_point(aes(color = .data[[subject_col]]), alpha = 0.3) +
    geom_line(aes(group = .data[[subject_col]], color = .data[[subject_col]]), alpha = 0.3) +
    theme_by_timepoint() +
...
create_longitudinal_cell_plots <- function(df, 
                                           timepoint_labels, 
                                           timepoints_pairs,
                                           cell_type_name = "cell",
                                           subplot_width = 4,
  ...
  df_copy <- as.data.table(df)
  
  # Copy percent column to expected variable name
  df_copy$cell_type_frac_total <- df_copy[[percent_col]]
  df_copy$subject <- df_copy[[subject_col]]
  df_copy$visit <- df_copy[[visit_col]]
  df_copy$cell <- df_copy[[cell_col]]
  
  # Convert visit to ordered factor with custom labels - use explicit level order
  level_order <- c("Y1 SA", "Y1 Day 0", "Y1 Day 7", "Y1 Day 90", 
                   "Y2 SA", "Y2 Day 0", "Y2 Day 7", "Y2 Day 90")
  df_copy$visit <- factor(timepoint_labels[df_copy$visit], levels = level_order)
  
  # Calculate grid dimensions based on number of cell types
  num_cells <- length(unique(df_copy$cell))
  ...
  create_figure <- function(pairs, title, use_duplicates = FALSE) {
    plotlist <- lapply(unique(df_copy$cell), function(x) {
      testCelltype_paired(df_copy, x, pairsOfTimes = pairs, size = stat_test_size, use_duplicates = use_duplicates)$Plot
    })
  ...
  for (i in seq_along(timepoints_pairs)) {
    ...
    plots[[year_name]] <- create_figure(timepoints_pairs[[i]], full_title, use_duplicates = use_duplicates)
  }
